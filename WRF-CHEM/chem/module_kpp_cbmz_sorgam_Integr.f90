MODULE cbmz_sorgam_Integrator
 USE cbmz_sorgam_Parameters
 USE cbmz_sorgam_Precision
 USE cbmz_sorgam_JacobianSP
  IMPLICIT NONE
  ...
  CHARACTER(LEN=50), PARAMETER, DIMENSION(-8:1) :: IERR_NAMES = (/ &
    ...
    'Success                                           ' /)
CONTAINS
SUBROUTINE cbmz_sorgam_INTEGRATE( TIN, TOUT, &
...
SUBROUTINE cbmz_sorgam_Rosenbrock(Y, FIX, RCONST, Tstart,Tend, &
  ...
  INTEGER :: Nfun,Njac,Nstp,Nacc,Nrej,Ndec,Nsol,Nsng
   ...
   ELSE
      ...
      CALL cbmz_sorgam_ros_ErrorMsg(-2,Tstart,ZERO,IERR)
      RETURN
   END IF
   IF (ICNTRL(4) == 0) THEN
      Max_no_steps = 100000
   ...
   ELSE
      ...
      CALL cbmz_sorgam_ros_ErrorMsg(-1,Tstart,ZERO,IERR)
      RETURN
   END IF
   Roundoff = cbmz_sorgam_WLAMCH('E')
   ...
   ELSE
      ...
      CALL cbmz_sorgam_ros_ErrorMsg(-3,Tstart,ZERO,IERR)
      RETURN
   END IF
   ...
   ELSE
      ...
      CALL cbmz_sorgam_ros_ErrorMsg(-4,Tstart,ZERO,IERR)
      RETURN
   END IF
   ...
   END IF
    DO i=1,UplimTol
      IF ( (AbsTol(i) <= ZERO) .OR. (RelTol(i) <= 10.0_dp*Roundoff) &
        ...
        CALL cbmz_sorgam_ros_ErrorMsg(-5,Tstart,ZERO,IERR)
        RETURN
      END IF
    END DO
   SELECT CASE (Method)
     CASE (1)
       CALL cbmz_sorgam_Ros2(ros_S, ros_A, ros_C, ros_M, ros_E, &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE (2)
       CALL cbmz_sorgam_Ros3(ros_S, ros_A, ros_C, ros_M, ros_E, &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE (3)
       CALL cbmz_sorgam_Ros4(ros_S, ros_A, ros_C, ros_M, ros_E, &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE (4)
       CALL cbmz_sorgam_Rodas3(ros_S, ros_A, ros_C, ros_M, ros_E, &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE (5)
       CALL cbmz_sorgam_Rodas4(ros_S, ros_A, ros_C, ros_M, ros_E, &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE DEFAULT
       ...
       RETURN
   END SELECT
   CALL cbmz_sorgam_ros_Integrator(Y,Tstart,Tend,Texit, &
        AbsTol, RelTol, &
...
CONTAINS
 SUBROUTINE cbmz_sorgam_ros_ErrorMsg(Code,T,H,IERR)
   USE cbmz_sorgam_Precision
   ...
   ENDIF
   PRINT *, "T=", T, "and H=", H
 END SUBROUTINE cbmz_sorgam_ros_ErrorMsg
 SUBROUTINE cbmz_sorgam_ros_Integrator (Y, Tstart, Tend, T, &
        AbsTol, RelTol, &
        ros_S, ros_M, ros_E, ros_A, ros_C, &
        ros_Alpha, ros_Gamma, ros_ELO, ros_NewF, &
        Autonomous, VectorTol, Max_no_steps, &
...
TimeLoop: DO WHILE ( (Direction > 0).AND.((T-Tend)+Roundoff <= ZERO) &
   ...
   IF ( Nstp > Max_no_steps ) THEN
      CALL cbmz_sorgam_ros_ErrorMsg(-6,T,H,IERR)
      RETURN
   END IF
   IF ( ((T+0.1_dp*H) == T).OR.(H <= Roundoff) ) THEN
      CALL cbmz_sorgam_ros_ErrorMsg(-7,T,H,IERR)
      RETURN
   END IF
   Hexit = H
   H = MIN(H,ABS(Tend-T))
   CALL cbmz_sorgam_FunTemplate(T,Y,Fcn0, RCONST, FIX, Nfun)
   IF (.NOT.Autonomous) THEN
      CALL cbmz_sorgam_ros_FunTimeDeriv ( T, Roundoff, Y, &
                Fcn0, dFdT, RCONST, FIX, Nfun )
   END IF
   CALL cbmz_sorgam_JacTemplate(T,Y,Jac0, FIX, Njac, RCONST)
UntilAccepted: DO
   CALL cbmz_sorgam_ros_PrepareMatrix(H,Direction,ros_Gamma(1), &
          Jac0,Ghimj,Pivot,Singular, Ndec, Nsng )
   IF (Singular) THEN
       CALL cbmz_sorgam_ros_ErrorMsg(-8,T,H,IERR)
       RETURN
   END IF
Stage: DO istage = 1, ros_S
       ioffset = NVAR*(istage-1)
       IF ( istage == 1 ) THEN
         CALL cbmz_sorgam_WCOPY(NVAR,Fcn0,1,Fcn,1)
       ELSEIF ( ros_NewF(istage) ) THEN
         CALL cbmz_sorgam_WCOPY(NVAR,Y,1,Ynew,1)
         DO j = 1, istage-1
           CALL cbmz_sorgam_WAXPY(NVAR,ros_A((istage-1)*(istage-2)/2+j), &
            K(NVAR*(j-1)+1),1,Ynew,1)
         END DO
         Tau = T + ros_Alpha(istage)*Direction*H
         CALL cbmz_sorgam_FunTemplate(Tau,Ynew,Fcn, RCONST, FIX, Nfun)
       END IF
       CALL cbmz_sorgam_WCOPY(NVAR,Fcn,1,K(ioffset+1),1)
       DO j = 1, istage-1
         HC = ros_C((istage-1)*(istage-2)/2+j)/(Direction*H)
         CALL cbmz_sorgam_WAXPY(NVAR,HC,K(NVAR*(j-1)+1),1,K(ioffset+1),1)
       END DO
       IF ((.NOT. Autonomous).AND.(ros_Gamma(istage).NE.ZERO)) THEN
         HG = Direction*H*ros_Gamma(istage)
         CALL cbmz_sorgam_WAXPY(NVAR,HG,dFdT,1,K(ioffset+1),1)
       END IF
       CALL cbmz_sorgam_ros_Solve(Ghimj, Pivot, K(ioffset+1), Nsol)
   END DO Stage
   CALL cbmz_sorgam_WCOPY(NVAR,Y,1,Ynew,1)
   DO j=1,ros_S
         CALL cbmz_sorgam_WAXPY(NVAR,ros_M(j),K(NVAR*(j-1)+1),1,Ynew,1)
   END DO
   CALL cbmz_sorgam_WSCAL(NVAR,ZERO,Yerr,1)
   DO j=1,ros_S
        CALL cbmz_sorgam_WAXPY(NVAR,ros_E(j),K(NVAR*(j-1)+1),1,Yerr,1)
   END DO
   Err = cbmz_sorgam_ros_ErrorNorm ( Y, Ynew, Yerr, AbsTol, RelTol, VectorTol )
   Fac = MIN(FacMax,MAX(FacMin,FacSafe/Err**(ONE/ros_ELO)))
   Hnew = H*Fac
   Nstp = Nstp+1
   IF ( (Err <= ONE).OR.(H <= Hmin) ) THEN
      Nacc = Nacc+1
      CALL cbmz_sorgam_WCOPY(NVAR,Ynew,1,Y,1)
      T = T + Direction*H
   ...
   END DO TimeLoop
   IERR = 1
  END SUBROUTINE cbmz_sorgam_ros_Integrator
  REAL(kind=dp) FUNCTION cbmz_sorgam_ros_ErrorNorm ( Y, Ynew, Yerr, &
   ...
   END DO
   Err = SQRT(Err/NVAR)
    cbmz_sorgam_ros_ErrorNorm = Err
  END FUNCTION cbmz_sorgam_ros_ErrorNorm
  SUBROUTINE cbmz_sorgam_ros_FunTimeDeriv ( T, Roundoff, Y, &
                Fcn0, dFdT, RCONST, FIX, Nfun )
   IMPLICIT NONE
   ...
   CALL cbmz_sorgam_FunTemplate(T+Delta,Y,dFdT, RCONST, FIX, Nfun)
   CALL cbmz_sorgam_WAXPY(NVAR,(-ONE),Fcn0,1,dFdT,1)
   CALL cbmz_sorgam_WSCAL(NVAR,(ONE/Delta),dFdT,1)
  END SUBROUTINE cbmz_sorgam_ros_FunTimeDeriv
  SUBROUTINE cbmz_sorgam_ros_PrepareMatrix ( H, Direction, gam, &
             Jac0, Ghimj, Pivot, Singular, Ndec, Nsng )
   IMPLICIT NONE
   ...
   Singular = .TRUE.
   DO WHILE (Singular)
     CALL cbmz_sorgam_WCOPY(LU_NONZERO,Jac0,1,Ghimj,1)
     CALL cbmz_sorgam_WSCAL(LU_NONZERO,(-ONE),Ghimj,1)
     ghinv = ONE/(Direction*H*gam)
     DO i=1,NVAR
       Ghimj(LU_DIAG(i)) = Ghimj(LU_DIAG(i))+ghinv
     END DO
     CALL cbmz_sorgam_ros_Decomp( Ghimj, Pivot, ising, Ndec )
     IF (ising == 0) THEN
        Singular = .FALSE.
     ELSE
        Nsng = Nsng+1
      ...
      END IF
   END DO
  END SUBROUTINE cbmz_sorgam_ros_PrepareMatrix
  SUBROUTINE cbmz_sorgam_ros_Decomp( A, Pivot, ising, Ndec )
   IMPLICIT NONE
   REAL(kind=dp), INTENT(INOUT) :: A(LU_NONZERO)
   INTEGER, INTENT(OUT) :: Pivot(NVAR), ising
   INTEGER, INTENT(INOUT) :: Ndec
CALL decomp_cbmz_sorgam ( A, ising )
   Pivot(1) = 1
   Ndec = Ndec + 1
  END SUBROUTINE cbmz_sorgam_ros_Decomp
  SUBROUTINE cbmz_sorgam_ros_Solve( A, Pivot, b, Nsol )
   IMPLICIT NONE
   REAL(kind=dp), INTENT(IN) :: A(LU_NONZERO)
   INTEGER, INTENT(IN) :: Pivot(NVAR)
   INTEGER, INTENT(INOUT) :: nsol
   REAL(kind=dp), INTENT(INOUT) :: b(NVAR)
   CALL cbmz_sorgam_KppSolve( A, b )
   Nsol = Nsol+1
  END SUBROUTINE cbmz_sorgam_ros_Solve
  SUBROUTINE cbmz_sorgam_Ros2 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
  ...
  IMPLICIT NONE
   ...
   CHARACTER(LEN=12), INTENT(OUT) :: ros_Name
    ...
    ros_Gamma(1) = g
    ros_Gamma(2) =-g
 END SUBROUTINE cbmz_sorgam_Ros2
  SUBROUTINE cbmz_sorgam_Ros3 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
           ros_Gamma,ros_NewF,ros_ELO,ros_Name)
  IMPLICIT NONE
  ...
  END SUBROUTINE cbmz_sorgam_Ros3
  SUBROUTINE cbmz_sorgam_Ros4 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
           ros_Gamma,ros_NewF,ros_ELO,ros_Name)
  IMPLICIT NONE
  ...
  END SUBROUTINE cbmz_sorgam_Ros4
  SUBROUTINE cbmz_sorgam_Rodas3 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
            ros_Gamma,ros_NewF,ros_ELO,ros_Name)
  IMPLICIT NONE
   INTEGER, PARAMETER :: S=4
   INTEGER, INTENT(OUT) :: ros_S
  ...
  END SUBROUTINE cbmz_sorgam_Rodas3
  SUBROUTINE cbmz_sorgam_Rodas4 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
             ros_Gamma,ros_NewF,ros_ELO,ros_Name)
  IMPLICIT NONE
   ...
   CHARACTER(LEN=12), INTENT(OUT) :: ros_Name
    ...
    ros_NewF(6) = .TRUE.
    ros_ELO = 4.0_dp
  END SUBROUTINE cbmz_sorgam_Rodas4
END SUBROUTINE cbmz_sorgam_Rosenbrock
SUBROUTINE cbmz_sorgam_FunTemplate( T, Y, Ydot, RCONST, FIX, Nfun )
   USE cbmz_sorgam_Parameters
   ...
   INTEGER :: Nfun
   CALL cbmz_sorgam_Fun( Y, FIX, RCONST, Ydot )
   Nfun = Nfun+1
END SUBROUTINE cbmz_sorgam_FunTemplate
SUBROUTINE cbmz_sorgam_JacTemplate( T, Y, Jcb, FIX, Njac, RCONST )
 USE cbmz_sorgam_Parameters
 USE cbmz_sorgam_Jacobian
    REAL(kind=dp) :: T, Y(NVAR)
    REAL(kind=dp) :: FIX(NFIX)
    REAL(kind=dp) :: RCONST(NREACT)
    INTEGER :: Njac
    ...
    REAL(kind=dp) :: Told
    CALL cbmz_sorgam_Jac_SP( Y, FIX, RCONST, Jcb )
    Njac = Njac+1
END SUBROUTINE cbmz_sorgam_JacTemplate
SUBROUTINE cbmz_sorgam_Fun ( V, F, RCT, Vdot )
  REAL(kind=dp) :: V(NVAR)
  REAL(kind=dp) :: F(NFIX)
...
END SUBROUTINE cbmz_sorgam_Fun
SUBROUTINE cbmz_sorgam_Jac_SP ( V, F, RCT, JVS )
  REAL(kind=dp) :: V(NVAR)
  REAL(kind=dp) :: F(NFIX)
  REAL(kind=dp) :: RCT(NREACT)
...
END SUBROUTINE cbmz_sorgam_Jac_SP
SUBROUTINE cbmz_sorgam_KppDecomp( JVS, IER )
      INTEGER :: IER
      ...
      DO k=1,NVAR
        ...
        DO kk = LU_CROW(k), LU_DIAG(k)-1
         ...
         END DO
      END DO
END SUBROUTINE cbmz_sorgam_KppDecomp
SUBROUTINE cbmz_sorgam_KppDecompCmplx( JVS, IER )
      INTEGER :: IER
      ...
      DO k=1,NVAR
        ...
        DO kk = LU_CROW(k), LU_DIAG(k)-1
         ...
         END DO
      END DO
END SUBROUTINE cbmz_sorgam_KppDecompCmplx
SUBROUTINE cbmz_sorgam_KppSolveIndirect( JVS, X )
      INTEGER i, j
      ...
      END DO
END SUBROUTINE cbmz_sorgam_KppSolveIndirect
SUBROUTINE cbmz_sorgam_KppSolveCmplx( JVS, X )
      INTEGER i, j
      ...
      DO i=NVAR,1,-1
        ...
        X(i) = sum/JVS(LU_DIAG(i));
      END DO
END SUBROUTINE cbmz_sorgam_KppSolveCmplx
SUBROUTINE cbmz_sorgam_KppSolve ( JVS, X )
  REAL(kind=dp) :: JVS(LU_NONZERO)
  REAL(kind=dp) :: X(NVAR)
...
END SUBROUTINE cbmz_sorgam_KppSolve
      SUBROUTINE cbmz_sorgam_WCOPY(N,X,incX,Y,incY)
      ...
      DO i = MP1,N,8
        ...
        Y(i + 7) = X(i + 7)
      END DO
      END SUBROUTINE cbmz_sorgam_WCOPY
      SUBROUTINE cbmz_sorgam_WAXPY(N,Alpha,X,incX,Y,incY)
      INTEGER i,incX,incY,M,MP1,N
      ...
      END DO
      END SUBROUTINE cbmz_sorgam_WAXPY
      SUBROUTINE cbmz_sorgam_WSCAL(N,Alpha,X,incX)
      ...
      ELSE
        ...
        END DO
      END IF
      END SUBROUTINE cbmz_sorgam_WSCAL
      REAL(kind=dp) FUNCTION cbmz_sorgam_WLAMCH( C )
      CHARACTER C
      INTEGER i
      ...
      IF (First) THEN
        ...
        DO i = 17, 80
          Eps = Eps*HALF
          CALL cbmz_sorgam_WLAMCH_ADD(ONE,Eps,Sum)
...
10 Eps = Eps*2
        i = i-1
      END IF
      cbmz_sorgam_WLAMCH = Eps
      END FUNCTION cbmz_sorgam_WLAMCH
      SUBROUTINE cbmz_sorgam_WLAMCH_ADD( A, B, Sum )
      REAL(kind=dp) A, B, Sum
      Sum = A + B
      END SUBROUTINE cbmz_sorgam_WLAMCH_ADD
      SUBROUTINE cbmz_sorgam_SET2ZERO(N,Y)
      INTEGER :: i,M,MP1,N
      REAL(kind=dp) :: Y(N)
      ...
      DO i = MP1,N,8
        ...
        Y(i + 7) = ZERO
      END DO
      END SUBROUTINE cbmz_sorgam_SET2ZERO
      REAL(kind=dp) FUNCTION cbmz_sorgam_WDOT (N, DX, incX, DY, incY)
      IMPLICIT NONE
      INTEGER :: N, incX, incY
      REAL(kind=dp) :: DX(N), DY(N)
      INTEGER :: i, IX, IY, M, MP1, NS
      cbmz_sorgam_WDOT = 0.0D0
      IF (N .LE. 0) RETURN
    ...
    5 IX = 1
      ...
      DO i = 1,N
        cbmz_sorgam_WDOT = cbmz_sorgam_WDOT + DX(IX)*DY(IY)
        IX = IX + incX
        IY = IY + incY
      END DO
      RETURN
   20 M = MOD(N,5)
      IF (M .EQ. 0) GO TO 40
      DO i = 1,M
         cbmz_sorgam_WDOT = cbmz_sorgam_WDOT + DX(i)*DY(i)
      END DO
      IF (N .LT. 5) RETURN
   40 MP1 = M + 1
      DO i = MP1,N,5
          cbmz_sorgam_WDOT = cbmz_sorgam_WDOT + DX(i)*DY(i) + DX(i+1)*DY(i+1) +&
                   DX(i+2)*DY(i+2) + &
   ...
   60 NS = N*incX
      DO i = 1,NS,incX
        cbmz_sorgam_WDOT = cbmz_sorgam_WDOT + DX(i)*DY(i)
      END DO
      END FUNCTION cbmz_sorgam_WDOT
   SUBROUTINE decomp_cbmz_sorgam( JVS, IER )
     IMPLICIT NONE
      INTEGER :: IER
  ...
  JVS( 625) = W( 65 )
  JVS( 626) = W( 66 )
   END SUBROUTINE decomp_cbmz_sorgam
END MODULE cbmz_sorgam_Integrator
