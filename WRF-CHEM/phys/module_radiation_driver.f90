...
CONTAINS
   SUBROUTINE radiation_driver ( &
              ...
              ,cubot,cutop,cupflag,cldfra_cup &
              ,shall &
              ,qc_ic_cup,qndrop_ic_cup &
              ,qi_ic_cup, chem &
              ...
              , TSWDN, TSWUP &
              , SSWDN, SSWUP &
              , CLDFRA,CLDFRA_MP_ALL,lradius,iradius &
              , ICLOUD_CU &
              ...
              ,SWUPT_NO_AER ,SWUPTC_NO_AER &
              ,SWDNT_NO_AER ,SWDNTC_NO_AER &
              ,SWUPB_NO_AER ,SWUPBC_NO_AER &
              ,SWDNB_NO_AER ,SWDNBC_NO_AER &
              ,SWUPT_NO_BC ,SWUPTC_NO_BC &
              ,SWDNT_NO_BC ,SWDNTC_NO_BC &
              ,SWUPB_NO_BC ,SWUPBC_NO_BC &
              ,SWDNB_NO_BC ,SWDNBC_NO_BC &
              ,SWUPT_NO_SO4 ,SWUPTC_NO_SO4 &
              ,SWDNT_NO_SO4 ,SWDNTC_NO_SO4 &
              ,SWUPB_NO_SO4 ,SWUPBC_NO_SO4 &
              ,SWDNB_NO_SO4 ,SWDNBC_NO_SO4 &
              ,SWUPT_NO_O3 ,SWUPTC_NO_O3 &
              ,SWDNT_NO_O3 ,SWDNTC_NO_O3 &
              ,SWUPB_NO_O3 ,SWUPBC_NO_O3 &
              ,SWDNB_NO_O3 ,SWDNBC_NO_O3 &
              ,LWUPT_NO_O3 ,LWUPTC_NO_O3 &
              ,LWDNT_NO_O3 ,LWDNTC_NO_O3 &
              ,LWUPB_NO_O3 ,LWUPBC_NO_O3 &
              ,LWDNB_NO_O3 ,LWDNBC_NO_O3 &
              ,LWCF &
              ,SWCF &
              ,OLR &
              ,aerodm, PINA, aodtot &
   ...
   REAL, DIMENSION( ims:ime, kms:kme, jms:jme ), &
        INTENT(INOUT ) :: cldfra_cup, &
                                                       ...
                                                       qi_ic_cup, &
                                                   qndrop_ic_cup
   LOGICAL, DIMENSION( ims:ime, jms:jme ), INTENT(IN) :: cupflag
   ...
   REAL, DIMENSION( ims:ime, jms:jme ), OPTIONAL, INTENT(INOUT) ::&
                        SWUPT_NO_AER, SWUPTC_NO_AER, SWDNT_NO_AER, &
                        SWDNTC_NO_AER, SWUPB_NO_AER, SWUPBC_NO_AER, &
                        SWDNB_NO_AER, SWDNBC_NO_AER, &
                        SWUPT_NO_BC, SWUPTC_NO_BC, SWDNT_NO_BC, &
                        SWDNTC_NO_BC, SWUPB_NO_BC, SWUPBC_NO_BC, &
                        SWDNB_NO_BC, SWDNBC_NO_BC, &
                        SWUPT_NO_SO4, SWUPTC_NO_SO4, SWDNT_NO_SO4, &
                        SWDNTC_NO_SO4, SWUPB_NO_SO4, SWUPBC_NO_SO4, &
                        SWDNB_NO_SO4, SWDNBC_NO_SO4, &
                        SWUPT_NO_O3, SWUPTC_NO_O3, SWDNT_NO_O3, &
                        SWDNTC_NO_O3, SWUPB_NO_O3, SWUPBC_NO_O3, &
                        SWDNB_NO_O3, SWDNBC_NO_O3, &
                        LWUPT_NO_O3, LWUPTC_NO_O3, LWDNT_NO_O3, &
                        LWDNTC_NO_O3, LWUPB_NO_O3, LWUPBC_NO_O3, &
                        LWDNB_NO_O3, LWDNBC_NO_O3
   ...
   REAL, DIMENSION( ims:ime, kms:kme, jms:jme ) :: qi_save,qc_save,CLDFRA_SAVE, qndrop_save
   REAL :: next_rad_time
   ...
   IF ( ( .NOT. decided ) .AND. &
      ...
      radtacttime = curr_secs + radt*60
   END IF
   stepradeffect = nint(radeffectdt*60.0/dt)
   stepradeffect = max(stepradeffect,1)
     ...
     IF ( PRESENT ( CLDFRA ) .AND. &
        ...
        IF ( cu_physics == KFCUPSCHEME ) THEN
          IF ( icloud_cu .EQ. 2 ) THEN
             ...
             DO k = kts,kte
             DO i = its,ite
                CLDFRA_SAVE(i,k,j)=CLDFRA(i,k,j)
                CLDFRA(i,k,j)=(1.-cldfra_cup(i,k,j))*CLDFRA(i,k,j)
                CLDFRA(i,k,j)=CLDFRA(i,k,j)+cldfra_cup(i,k,j)
                CLDFRA(i,k,j)=AMIN1(1.0,CLDFRA(i,k,j))
                qndrop_save(i,k,j)=qndrop(i,k,j)
                qndrop(i,k,j) = qndrop(i,k,j) + qndrop_ic_cup(i,k,j)*cldfra_cup(i,k,j)
                qc_save(i,k,j)=qc(i,k,j)
                qc(i,k,j) = qc(i,k,j)+qc_ic_cup(i,k,j)*cldfra_cup(i,k,j)
                qi_save(i,k,j)=qi(i,k,j)
                qi(i,k,j) = qi(i,k,j)+qi_ic_cup(i,k,j)*cldfra_cup(i,k,j)
                qi(i,k,j)=AMAX1(0.0,qi(i,k,j))
             ENDDO
             ENDDO
             ENDDO
          ENDIF
...
'arguments not present for calling cam radiation' )
        ...
        CASE (RRTMG_LWSCHEME)
             ...
             CALL RRTMG_LWRAD( &
                  RTHRATENLW=RTHRATEN, &
                  LWUPT=LWUPT_NO_O3,LWUPTC=LWUPTC_NO_O3, &
                  LWDNT=LWDNT_NO_O3,LWDNTC=LWDNTC_NO_O3, &
                  LWUPB=LWUPB_NO_O3,LWUPBC=LWUPBC_NO_O3, &
                  LWDNB=LWDNB_NO_O3,LWDNBC=LWDNBC_NO_O3, &
...
'arguments not present for calling cam radiation' )
        ...
        CASE (RRTMG_SWSCHEME)
             ...
             CALL RRTMG_SWRAD( &
                     ...
                     SWUPT=SWUPT_NO_AER,SWUPTC=SWUPTC_NO_AER, &
                     SWDNT=SWDNT_NO_AER,SWDNTC=SWDNTC_NO_AER, &
                     SWUPB=SWUPB_NO_AER,SWUPBC=SWUPBC_NO_AER, &
                     SWDNB=SWDNB_NO_AER,SWDNBC=SWDNBC_NO_AER, &
                     SWCF=SWCF,GSW=GSW, &
             ...
             CALL RRTMG_SWRAD( &
                     RTHRATENSW=RTHRATENSW, &
                     SWUPT=SWUPT_NO_BC,SWUPTC=SWUPTC_NO_BC, &
                     SWDNT=SWDNT_NO_BC,SWDNTC=SWDNTC_NO_BC, &
                     SWUPB=SWUPB_NO_BC,SWUPBC=SWUPBC_NO_BC, &
                     SWDNB=SWDNB_NO_BC,SWDNBC=SWDNBC_NO_BC, &
                     SWCF=SWCF,GSW=GSW, &
                     XTIME=XTIME,GMT=GMT,XLAT=XLAT,XLONG=XLONG, &
                     RADT=RADT,DEGRAD=DEGRAD,DECLIN=DECLIN, &
                     COSZR=COSZR,JULDAY=JULDAY,SOLCON=SOLCON, &
                     ...
                     RE_CLOUD=re_cloud,RE_ICE=re_ice,RE_SNOW=re_snow, &
                     has_reqc=has_reqc,has_reqi=has_reqi,has_reqs=has_reqs, &
                     TAUAER300=tauaer300_no_bc, &
                     TAUAER400=tauaer400_no_bc, &
                     TAUAER600=tauaer600_no_bc, &
                     TAUAER999=tauaer999_no_bc, &
                     GAER300=gaer300_no_bc,GAER400=gaer400_no_bc, &
                     GAER600=gaer600_no_bc,GAER999=gaer999_no_bc, &
                     WAER300=waer300_no_bc,WAER400=waer400_no_bc, &
                     WAER600=waer600_no_bc,WAER999=waer999_no_bc, &
                     aer_ra_feedback=aer_ra_feedback, &
                     progn=progn, &
                     QNDROP3D=qndrop,F_QNDROP=f_qndrop, &
                     IDS=ids,IDE=ide, JDS=jds,JDE=jde, KDS=kds,KDE=kde,&
             ...
             CALL RRTMG_SWRAD( &
                     RTHRATENSW=RTHRATENSW, &
                     SWUPT=SWUPT_NO_O3,SWUPTC=SWUPTC_NO_O3, &
                     SWDNT=SWDNT_NO_O3,SWDNTC=SWDNTC_NO_O3, &
                     SWUPB=SWUPB_NO_O3,SWUPBC=SWUPBC_NO_O3, &
                     SWDNB=SWDNB_NO_O3,SWDNBC=SWDNBC_NO_O3, &
                     SWCF=SWCF,GSW=GSW, &
                     XTIME=XTIME,GMT=GMT,XLAT=XLAT,XLONG=XLONG, &
                     RADT=RADT,DEGRAD=DEGRAD,DECLIN=DECLIN, &
                     COSZR=COSZR,JULDAY=JULDAY,SOLCON=SOLCON, &
...
wrf_err_message )
     ...
     IF (sw_physics .gt. 0 .and. .not.gfdl_sw) THEN
        ...
        ENDDO
     ENDIF
     if ((sw_physics .ne. rrtmg_swscheme) .and. (sw_physics .ne. goddardswscheme) .and. cu_physics.ne. KFCUPSCHEME) then
        do j=jts,jte
           do i=its,ite
     ...
     IF ( cu_physics == KFCUPSCHEME ) THEN
         ...
         DO i=its,ite
           ...
           CLDFRA(i,k,j)=CLDFRA_SAVE(i,k,j)
           qndrop(i,k,j)=qndrop_save(i,k,j)
         ENDDO
         ENDDO
         ENDDO
         ENDIF
 ...
 if (swint_opt .eq. 1) then
   ...
   ENDIF
   IF (do_radeffect) THEN
   IF(stepradeffect .EQ. 1) THEN
     radeff_dt = dt
   ELSE
     radeff_dt = radeffectdt * 60.0
   ENDIF
   IF(PRESENT(LWUPTC))THEN
   !$OMP PARALLEL DO &
   !$OMP PRIVATE ( ij ,i,j,k,its,ite,jts,jte)
   DO ij = 1 , num_tiles
     ...
     jte = j_end(ij)
        DO j=jts,jte
        DO i=its,ite
           ACLWUPT_ALL(I,J) = ACLWUPT_ALL(I,J) + LWUPT(I,J)*RADEFF_DT
           ACLWUPTC_ALL(I,J) = ACLWUPTC_ALL(I,J) + LWUPTC(I,J)*RADEFF_DT
           ACLWDNT_ALL(I,J) = ACLWDNT_ALL(I,J) + LWDNT(I,J)*RADEFF_DT
           ACLWDNTC_ALL(I,J) = ACLWDNTC_ALL(I,J) + LWDNTC(I,J)*RADEFF_DT
           ACLWUPB_ALL(I,J) = ACLWUPB_ALL(I,J) + LWUPB(I,J)*RADEFF_DT
           ACLWUPBC_ALL(I,J) = ACLWUPBC_ALL(I,J) + LWUPBC(I,J)*RADEFF_DT
           ACLWDNB_ALL(I,J) = ACLWDNB_ALL(I,J) + LWDNB(I,J)*RADEFF_DT
           ACLWDNBC_ALL(I,J) = ACLWDNBC_ALL(I,J) + LWDNBC(I,J)*RADEFF_DT
           ACLWUPT_O3(I,J) = ACLWUPT_O3(I,J) + (LWUPT(I,J)*RADEFF_DT - LWUPT_NO_O3(I,J)*RADEFF_DT)
           ACLWUPTC_O3(I,J) = ACLWUPTC_O3(I,J) + (LWUPTC(I,J)*RADEFF_DT - LWUPTC_NO_O3(I,J)*RADEFF_DT)
           ACLWDNT_O3(I,J) = ACLWDNT_O3(I,J) + (LWDNT(I,J)*RADEFF_DT - LWDNT_NO_O3(I,J)*RADEFF_DT)
           ACLWDNTC_O3(I,J) = ACLWDNTC_O3(I,J) + (LWDNTC(I,J)*RADEFF_DT - LWDNTC_NO_O3(I,J)*RADEFF_DT)
           ACLWUPB_O3(I,J) = ACLWUPB_O3(I,J) + (LWUPB(I,J)*RADEFF_DT - LWUPB_NO_O3(I,J)*RADEFF_DT)
           ACLWUPBC_O3(I,J) = ACLWUPBC_O3(I,J) + (LWUPBC(I,J)*RADEFF_DT - LWUPBC_NO_O3(I,J)*RADEFF_DT)
           ACLWDNB_O3(I,J) = ACLWDNB_O3(I,J) + (LWDNB(I,J)*RADEFF_DT - LWDNB_NO_O3(I,J)*RADEFF_DT)
           ACLWDNBC_O3(I,J) = ACLWDNBC_O3(I,J) + (LWDNBC(I,J)*RADEFF_DT - LWDNBC_NO_O3(I,J)*RADEFF_DT)
        ENDDO
        ENDDO
   ENDDO
   !$OMP END PARALLEL DO
   ...
   DO ij = 1 , num_tiles
     ...
     jte = j_end(ij)
        DO j=jts,jte
        DO i=its,ite
           ACSWUPT_ALL(I,J) = ACSWUPT_ALL(I,J) + SWUPT(I,J)*RADEFF_DT
           ACSWUPTC_ALL(I,J) = ACSWUPTC_ALL(I,J) + SWUPTC(I,J)*RADEFF_DT
           ACSWDNT_ALL(I,J) = ACSWDNT_ALL(I,J) + SWDNT(I,J)*RADEFF_DT
           ACSWDNTC_ALL(I,J) = ACSWDNTC_ALL(I,J) + SWDNTC(I,J)*RADEFF_DT
           ACSWUPB_ALL(I,J) = ACSWUPB_ALL(I,J) + SWUPB(I,J)*RADEFF_DT
           ACSWUPBC_ALL(I,J) = ACSWUPBC_ALL(I,J) + SWUPBC(I,J)*RADEFF_DT
           ACSWDNB_ALL(I,J) = ACSWDNB_ALL(I,J) + SWDNB(I,J)*RADEFF_DT
           ACSWDNBC_ALL(I,J) = ACSWDNBC_ALL(I,J) + SWDNBC(I,J)*RADEFF_DT
           ACSWUPT_AER(I,J) = ACSWUPT_AER(I,J) + (SWUPT(I,J)*RADEFF_DT - SWUPT_NO_AER(I,J)*RADEFF_DT)
           ACSWUPTC_AER(I,J) = ACSWUPTC_AER(I,J) + (SWUPTC(I,J)*RADEFF_DT - SWUPTC_NO_AER(I,J)*RADEFF_DT)
           ACSWDNT_AER(I,J) = ACSWDNT_AER(I,J) + (SWDNT(I,J)*RADEFF_DT - SWDNT_NO_AER(I,J)*RADEFF_DT)
           ACSWDNTC_AER(I,J) = ACSWDNTC_AER(I,J) + (SWDNTC(I,J)*RADEFF_DT - SWDNTC_NO_AER(I,J)*RADEFF_DT)
           ACSWUPB_AER(I,J) = ACSWUPB_AER(I,J) + (SWUPB(I,J)*RADEFF_DT - SWUPB_NO_AER(I,J)*RADEFF_DT)
           ACSWUPBC_AER(I,J) = ACSWUPBC_AER(I,J) + (SWUPBC(I,J)*RADEFF_DT - SWUPBC_NO_AER(I,J)*RADEFF_DT)
           ACSWDNB_AER(I,J) = ACSWDNB_AER(I,J) + (SWDNB(I,J)*RADEFF_DT - SWDNB_NO_AER(I,J)*RADEFF_DT)
           ACSWDNBC_AER(I,J) = ACSWDNBC_AER(I,J) + (SWDNBC(I,J)*RADEFF_DT - SWDNBC_NO_AER(I,J)*RADEFF_DT)
           ACSWUPT_BC(I,J) = ACSWUPT_BC(I,J) + (SWUPT(I,J)*RADEFF_DT - SWUPT_NO_BC(I,J)*RADEFF_DT)
           ACSWUPTC_BC(I,J) = ACSWUPTC_BC(I,J) + (SWUPTC(I,J)*RADEFF_DT - SWUPTC_NO_BC(I,J)*RADEFF_DT)
           ACSWDNT_BC(I,J) = ACSWDNT_BC(I,J) + (SWDNT(I,J)*RADEFF_DT - SWDNT_NO_BC(I,J)*RADEFF_DT)
           ACSWDNTC_BC(I,J) = ACSWDNTC_BC(I,J) + (SWDNTC(I,J)*RADEFF_DT - SWDNTC_NO_BC(I,J)*RADEFF_DT)
           ACSWUPB_BC(I,J) = ACSWUPB_BC(I,J) + (SWUPB(I,J)*RADEFF_DT - SWUPB_NO_BC(I,J)*RADEFF_DT)
           ACSWUPBC_BC(I,J) = ACSWUPBC_BC(I,J) + (SWUPBC(I,J)*RADEFF_DT - SWUPBC_NO_BC(I,J)*RADEFF_DT)
           ACSWDNB_BC(I,J) = ACSWDNB_BC(I,J) + (SWDNB(I,J)*RADEFF_DT - SWDNB_NO_BC(I,J)*RADEFF_DT)
           ACSWDNBC_BC(I,J) = ACSWDNBC_BC(I,J) + SWDNBC(I,J)*RADEFF_DT - SWDNBC_NO_BC(I,J)*RADEFF_DT
           ACSWUPT_SO4(I,J) = ACSWUPT_SO4(I,J) + (SWUPT(I,J)*RADEFF_DT - SWUPT_NO_SO4(I,J)*RADEFF_DT)
           ACSWUPTC_SO4(I,J) = ACSWUPTC_SO4(I,J) + (SWUPTC(I,J)*RADEFF_DT - SWUPTC_NO_SO4(I,J)*RADEFF_DT)
           ACSWDNT_SO4(I,J) = ACSWDNT_SO4(I,J) + (SWDNT(I,J)*RADEFF_DT - SWDNT_NO_SO4(I,J)*RADEFF_DT)
           ACSWDNTC_SO4(I,J) = ACSWDNTC_SO4(I,J) + (SWDNTC(I,J)*RADEFF_DT - SWDNTC_NO_SO4(I,J)*RADEFF_DT)
           ACSWUPB_SO4(I,J) = ACSWUPB_SO4(I,J) + (SWUPB(I,J)*RADEFF_DT - SWUPB_NO_SO4(I,J)*RADEFF_DT)
           ACSWUPBC_SO4(I,J) = ACSWUPBC_SO4(I,J) + (SWUPBC(I,J)*RADEFF_DT - SWUPBC_NO_SO4(I,J)*RADEFF_DT)
           ACSWDNB_SO4(I,J) = ACSWDNB_SO4(I,J) + (SWDNB(I,J)*RADEFF_DT - SWDNB_NO_SO4(I,J)*RADEFF_DT)
           ACSWDNBC_SO4(I,J) = ACSWDNBC_SO4(I,J) + SWDNBC(I,J)*RADEFF_DT - SWDNBC_NO_SO4(I,J)*RADEFF_DT
           ACSWUPT_O3(I,J) = ACSWUPT_O3(I,J) + (SWUPT(I,J)*RADEFF_DT - SWUPT_NO_O3(I,J)*RADEFF_DT)
           ACSWUPTC_O3(I,J) = ACSWUPTC_O3(I,J) + (SWUPTC(I,J)*RADEFF_DT - SWUPTC_NO_O3(I,J)*RADEFF_DT)
           ACSWDNT_O3(I,J) = ACSWDNT_O3(I,J) + (SWDNT(I,J)*RADEFF_DT - SWDNT_NO_O3(I,J)*RADEFF_DT)
           ACSWDNTC_O3(I,J) = ACSWDNTC_O3(I,J) + (SWDNTC(I,J)*RADEFF_DT - SWDNTC_NO_O3(I,J)*RADEFF_DT)
           ACSWUPB_O3(I,J) = ACSWUPB_O3(I,J) + (SWUPB(I,J)*RADEFF_DT - SWUPB_NO_O3(I,J)*RADEFF_DT)
           ACSWUPBC_O3(I,J) = ACSWUPBC_O3(I,J) + (SWUPBC(I,J)*RADEFF_DT - SWUPBC_NO_O3(I,J)*RADEFF_DT)
           ACSWDNB_O3(I,J) = ACSWDNB_O3(I,J) + (SWDNB(I,J)*RADEFF_DT - SWDNB_NO_O3(I,J)*RADEFF_DT)
           ACSWDNBC_O3(I,J) = ACSWDNBC_O3(I,J) + SWDNBC(I,J)*RADEFF_DT - SWDNBC_NO_O3(I,J)*RADEFF_DT
        ENDDO
        ENDDO
   ENDDO
   !$OMP END PARALLEL DO
...
CALL HALO_TOPOSHAD_sub ( grid, &
  ...
  ips, ipe, jps, jpe, kps, kpe )
   ...
   REAL, DIMENSION( ims:ime, kms:kme, jms:jme ), INTENT(INOUT), optional :: &
                                                         cldfra_cup
   LOGICAL,DIMENSION(ims:ime, jms:jme), INTENT(IN),optional :: cupflag
   LOGICAL,INTENT(IN),optional :: shallowcu_forced_ra
   ...
   INTEGER:: i,j,k
     ...
     ELSE
        ...
        ENDDO
     ENDIF
     if(present(cu_physics)) then
        if( cu_physics == KFCUPSCHEME ) then
           DO j = jts,jte
              DO k = kts,kte
                 DO i = its,ite
                    if( k >= int(cubot(i,j)) .and. k <= int(cutop(i,j)) .and. (int(shall(i,j)) .le.1)) then
                       CLDFRA(i,k,j) = cldfra_cup(i,k,j)
                       print*,'cldfra=cldfra_cup',CLDFRA(i,k,j),cldfra_cup(i,k,j),i,j,k
   ...
   REAL , PARAMETER :: ep_2=r_d/r_v
    ...
    DO i = its,ite
      ...
      IF (QCLD .LT. QCLDMIN .and. shall(i,j) .gt. 1) THEN
        CLDFRA(i,k,j)=0.
     ELSEIF(RHUM.GE.RHGRID .and. shall(i,j) .gt. 1) THEN
        CLDFRA(i,k,j)=1.
      ELSE
