MODULE module_mp_nssl_2mom
  ...
  real , private :: ehw0 = 1.0
  real , private :: erw0 = 1.0
  real , private :: ehlw0 = 1.0
  ...
  real bx(lc:lqmx)
      ...
      parameter( xvhmn=0.523599*(0.3e-3)**3, xvhmx=0.523599*(10.e-3)**3 )
...
'nssl_2mom_init: Invalid value of ipctmp' )
    ENDIF
     ...
     & ((1 + alphahl)*(2 + alphahl)*(3 + alphahl)*(2. + xnu(lr)))
      ...
      ventrn = Gamma(rnu + 1.5 + br/6.)/Gamma(rnu + 1.)
...
29998 continue
 ...
 1100 continue
     ...
     & (alpha(mgs,lr) + 2.)*xv(mgs,lc)*xv(mgs,lr)**2/(alpha(mgs,lr) + 1.))/rho0(mgs)
         ENDIF
        ENDIF
       ENDIF
     ...
     & + gf3*xdia(mgs,lr,2) )
        ENDIF
       ELSE
        IF ( 0.5*xdia(mgs,lr,1) .gt. rh(mgs) ) THEN
        IF ( 0.5*xdia(mgs,lr,1) .gt. rwradmn ) THEN
       ...
       ELSEIF ( ipconc .ge. 3 ) THEN
         ...
         IF ( crfrz(mgs) .gt. 0.0 ) THEN
           ...
           ELSE
             qrfrzf(mgs) = qrfrz(mgs)
            IF ( ibfr .le. 1 ) THEN
             crfrzf(mgs) = Min(crfrz(mgs), qrfrz(mgs)/(bfnu*1.0*vr1mm*1000.0)*rho0(mgs) )
            ELSEIF ( ibfr .eq. 5 ) THEN
             crfrzf(mgs) = Min(crfrz(mgs), qrfrz(mgs)/(bfnu*vfrz*1000.0)*rho0(mgs) )*rzxh(mgs)
            ELSE
             crfrzf(mgs) = Min(crfrz(mgs), qrfrz(mgs)/(bfnu*vfrz*1000.0)*rho0(mgs) )
            ENDIF
           ENDIF
          IF ( lvol(lh) .gt. 1 ) THEN
     ...
     IF ( chmlr(mgs) < 0.0 ) THEN
      IF ( ihmlt .eq. 1 ) THEN
        chmlrr(mgs) = Max( chmlr(mgs), rho0(mgs)*qhmlr(mgs)/(xdn(mgs,lr)*vmlt) )
      ...
      IF ( ihmlt .eq. 1 ) THEN
        chlmlrr(mgs) = Max( chlmlr(mgs), rho0(mgs)*qhlmlr(mgs)/(xdn(mgs,lr)*vmlt) )
      ELSEIF ( ihmlt .eq. 2 ) THEN
     ...
     & + fwet2(mgs)*(qhlaci(mgs) + qhlacs(mgs)) )
      ...
      IF ( temg(mgs) < tfr ) THEN
         chshrr(mgs) = Min( chshr(mgs), rho0(mgs)*qhshr(mgs)/(xdn0(lr)*vr4p5mm) )
      ELSE
         chshrr(mgs) = Min( chshr(mgs), rho0(mgs)*qhshr(mgs)/(xdn(mgs,lr)*xvmx(lr)) )
      ENDIF
      chlshr(mgs) = 0.0
      chlshrr(mgs) = 0.0
      IF ( lhl .gt. 1 ) THEN
        ...
        IF ( temg(mgs) < tfr ) THEN
          chlshrr(mgs) = Min( chlshr(mgs), rho0(mgs)*qhlshr(mgs)/(xdn0(lr)*vr4p5mm) )
        ELSE
           chlshrr(mgs) = Min( chlshr(mgs), rho0(mgs)*qhlshr(mgs)/(xdn(mgs,lr)*xvmx(lr)) )
        ENDIF
      ENDIF
      end do
      end if
