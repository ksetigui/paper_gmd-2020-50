...
advh_t,advz_t,emis_ant,eghg_bio,emis_dust,emis_seas,emis_vol,ebu,ebu_in,emis_aircraft,ext_coef,bscat_coef,asym_par,conv_ct, &
  ...
  ijde = max(ide, jde)
   ...
   chem_select: SELECT CASE(config_flags%chem_opt)
     ...
     CASE (SAPRC99_MOSAIC_4BIN_VBS9_KPP)
       CALL wrf_debug(15,'calling saprc99_mosaic_4bin_vbs9_kpp from chem_driver')
       haveaer = .false.
       haveaer = .false.
...
CALL kpp_mechanism_driver (chem, &
   ...
   vdrog3, ldrog, vdrog3_vbs, ldrog_vbs, &
             ...
             grid%ph_n2o, grid%ph_pooh, grid%ph_mpan, grid%ph_mvk, &
             grid%ph_etooh, grid%ph_prooh, grid%ph_onitr, grid%ph_acetol, &
             grid%ph_glyald, grid%ph_hyac, grid%ph_mek, grid%ph_open, &
             grid%ph_gly, grid%ph_acetp, grid%ph_xooh, grid%ph_isooh, &
             grid%ph_alkooh, grid%ph_mekooh, grid%ph_tolooh, grid%ph_terpooh, &
   ids,ide, jds,jde, kds,kde, &
   ims,ime, jms,jme, kms,kme, &
   its,ite,jts,jte,kts,kte)
          if( config_flags%chem_opt == MOZART_KPP .or. &
...
if ( cam_mam_aerosols ) &
        ...
        call cloudchem_driver( &
               ...
               p8w,grid%prain3d,scalar,grid%dvmrdt_sv13d,grid%dvmrcwdt_sv13d, grid%f_ice_phy, &
 ...
 if (config_flags%wetscav_onoff > 0) then
        call wetscav_driver (grid%id,ktau,grid%dt,grid%ktauc,config_flags,dtstepc, &
              ...
              grid%cldfra_cup, grid%precr_cup, grid%preci_cup, &
