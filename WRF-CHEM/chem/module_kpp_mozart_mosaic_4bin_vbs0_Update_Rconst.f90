MODULE mozart_mosaic_4bin_vbs0_UpdateRconstWRF
  USE mozart_mosaic_4bin_vbs0_Parameters
  IMPLICIT NONE
CONTAINS
SUBROUTINE mozart_mosaic_4bin_vbs0_Update_RCONST( &
   rh, &
   nume,den, &
                j, nj, &
                RCONST, &
  ...
  RCONST(197) = (1.25D-11)
  RCONST(198) = (1.25D-11)
  RCONST(199) = (Keff(2.50D-11,-408.0_dp,0.0_dp,TEMP,nume,den,0.0104_dp,0.0078_dp))
  RCONST(200) = (Keff(1.83D-11,-449.0_dp,0.0_dp,TEMP,nume,den,0.036_dp,0.2065_dp))
END SUBROUTINE mozart_mosaic_4bin_vbs0_Update_RCONST
...
END FUNCTION usr26
    ...
    Keff2 = C0*(y1*X1/(X1 + X2 + 1.0e-35) &
       +y2*(1-X1/(X1 + X2 + 1.0e-35 )))
    END FUNCTION Keff2
END MODULE mozart_mosaic_4bin_vbs0_UpdateRconstWRF
