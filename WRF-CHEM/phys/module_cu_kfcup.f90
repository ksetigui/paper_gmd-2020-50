               ...
               do ipert = 1,numBins
                  ...
                  if( ishall == 0 ) then
                    ...
                    do k = kts, kte
                     ...
                     cubot_shall = cubot_shall + z1d(cubot(i,j))*jfd(ipert,jpert)
                     cutop_shall = cutop_shall + z1d(cutop(i,j))*jfd(ipert,jpert)
...
main_test_on_deep_shall_freq: &
               if( cumDeepFreq > minDeepFreq ) then
                  ...
                  qi_ic_cup(i,kts:kte,j) = qi_ic_deep(kts:kte)
                  precr_cup(i,kts:kte,j) = precr_1d(kts:kte)
                  preci_cup(i,kts:kte,j) = preci_1d(kts:kte)
                  ...
                  qi_ic_cup(i,kts:kcutop,j) = qi_ic_shall(kts:kcutop)
                  precr_cup(i,kts:kcutop,j) = precr_1d(kts:kcutop)
                  preci_cup(i,kts:kcutop,j) = preci_1d(kts:kcutop)
...
'QG, QG(NK).LT.0')
        ...
        DO NK=1,LTOP
          ...
          QRG(NK)=QRPA(NK)
          QSG(NK)=QSPA(NK)
          PRECR_1D(NK)=QRG(NK)/DTIME/DXSQ/EMSD(NK)
          PRECI_1D(NK)=QSG(NK)/DTIME/DXSQ/EMSD(NK)
        ENDDO
        DO NK=LTOP-1,1,-1
          PRECR_1D(NK)=PRECR_1D(NK+1)+PRECR_1D(NK)
          PRECI_1D(NK)=PRECI_1D(NK+1)+PRECI_1D(NK)
        ENDDO
       IF(IPRNT)THEN
...
SUBROUTINE cupSlopeSigma(dx, psfc, p, rho, dz8w, z, ht, &
  ...
  if( abs(qfxin) < 1./xlv ) then
   ...
   if( kpbl == 0 ) call wrf_error_fatal3("<stdin>",4879,&
"CuP ERROR: kpbl==0")
