...
CONTAINS
    subroutine emissions_driver(id,ktau,dtstep,DX, &
         ...
         emis_vol,tsk,erod,g,emis_seas,emis_dust,tracer, &
         ...
         clayfrac,sandfrac,dust_alpha,dust_gamma,dust_smtune, &
         snowh,zs, &
  ...
  REAL, DIMENSION( ims:ime, config_flags%num_soil_layers, jms:jme ) , &
   ...
   integer, intent(in) :: current_month
     ...
     REAL, DIMENSION( ims:ime, jms:jme ), INTENT(IN ) :: htop
      ...
      CASE (GOCART_SIMPLE,MOZCART_KPP,GOCARTRADM2,GOCARTRACM_KPP,GOCARTRADM2_KPP)
      ...
      CASE (DUSTGOCARTAFWA)
        ...
        call gocart_dust_afwa_driver(ktau,dtstep,config_flags,julday,alt,t_phy,moist,u_phy, &
         ...
         ivgtyp,isltyp,vegfra,xland,xlat,xlong,gsw,dx,g,emis_dust, &
         ust,znt,clayfrac,sandfrac,dust_alpha,dust_gamma,dust_smtune, &
         ids,ide, jds,jde, kds,kde, &
         ims,ime, jms,jme, kms,kme, &
         its,ite, jts,jte, kts,kte )
      CASE DEFAULT
      ...
      CASE (DMSGOCART)
        ...
        call gocart_dmsemis(dtstep,config_flags,alt,t_phy,u_phy, &
         ...
         its,ite, jts,jte, kts,kte )
      CASE (DMSMOSAIC)
        CALL wrf_debug(15,'MOSAIC dms emissions')
        call mosaic_dmsemis( id, dtstep, dz8w, config_flags, &
    ...
    fire_select: SELECT CASE(config_flags%biomass_burn_opt)
     CASE (BIOMASSB,BIOMASSB_MOZC,BIOMASSB_MOZ,BIOMASSB_SAPRC,BIOMASSB_GHG)
    ...
    CASE (RADM2, RADM2_KPP, RADM2SORG, RADM2SORG_AQ, RADM2SORG_AQCHEM, RADM2SORG_KPP, &
          ...
          RACM_SOA_VBS_KPP, RACM_ESRLSORG_KPP, MOZART_KPP, MOZCART_KPP, MOZART_MOSAIC_4BIN_VBS0_KPP, CRIMECH_KPP, &
    ...
    IF(config_flags%emiss_opt == 10 ) then
       do j=jts,jte
          do i=its,ite
             do k=kts,min(config_flags%kemit,kte-ksub)
                conv = 4.828e-4/rho_phy(i,k,j)*dtstep/(dz8w(i,k,j)*60.)
                chem(i,k,j,p_voca) = chem(i,k,j,p_voca) &
                     +emis_ant(i,k,j,p_e_co_a)*conv*0.08*28./250.
                chem(i,k,j,p_vocbb) = chem(i,k,j,p_vocbb) &
                     +emis_ant(i,k,j,p_e_co_bb)*conv*0.08*28./250.
             end do
          end do
       end do
     endif
    CASE (CBMZ, CBMZ_BB, CBMZ_BB_KPP, CBMZ_MOSAIC_4BIN, CBMZ_MOSAIC_8BIN, &
          ...
          CBMZ_SORGAM_KPP,CBMZ_MOSAIC_KPP, CBMZ_MOSAIC_8BIN_VBS9_KPP,CBMZ_MOSAIC_4BIN_VBS9_KPP,&
...
CASE (SAPRC99_MOSAIC_4BIN_VBS9_KPP)
     if(config_flags%emiss_opt == 13 ) then
       do j=jts,jte
          do i=its,ite
             do k=kts,min(config_flags%kemit,kte-ksub)
        ...
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*0.105
        chem(i,k,j,p_pcg2_b_c) = chem(i,k,j,p_pcg2_b_c) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*0.211
        chem(i,k,j,p_pcg3_b_c) = chem(i,k,j,p_pcg3_b_c) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*0.316
        chem(i,k,j,p_pcg4_b_c) = chem(i,k,j,p_pcg4_b_c) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*0.491
        chem(i,k,j,p_pcg5_b_c) = chem(i,k,j,p_pcg5_b_c) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*0.632
        chem(i,k,j,p_pcg6_b_c) = chem(i,k,j,p_pcg6_b_c) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*1.053
        chem(i,k,j,p_pcg7_b_c) = chem(i,k,j,p_pcg7_b_c) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*1.404
        chem(i,k,j,p_pcg8_b_c) = chem(i,k,j,p_pcg8_b_c) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*1.755
        chem(i,k,j,p_pcg9_b_c) = chem(i,k,j,p_pcg9_b_c) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*2.808
        chem(i,k,j,p_pcg1_f_c) = chem(i,k,j,p_pcg1_f_c) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*0.269
        chem(i,k,j,p_pcg2_f_c) = chem(i,k,j,p_pcg2_f_c) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*0.199
        chem(i,k,j,p_pcg3_f_c) = chem(i,k,j,p_pcg3_f_c) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*0.304
        chem(i,k,j,p_pcg4_f_c) = chem(i,k,j,p_pcg4_f_c) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*0.468
        chem(i,k,j,p_pcg5_f_c) = chem(i,k,j,p_pcg5_f_c) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*0.597
        chem(i,k,j,p_pcg6_f_c) = chem(i,k,j,p_pcg6_f_c) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*1.006
        chem(i,k,j,p_pcg7_f_c) = chem(i,k,j,p_pcg7_f_c) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*1.369
        chem(i,k,j,p_pcg8_f_c) = chem(i,k,j,p_pcg8_f_c) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*1.755
        chem(i,k,j,p_pcg9_f_c) = chem(i,k,j,p_pcg9_f_c) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*2.808
        chem(i,k,j,p_pcg1_b_o) = chem(i,k,j,p_pcg1_b_o) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*0.036
        chem(i,k,j,p_pcg2_b_o) = chem(i,k,j,p_pcg2_b_o) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*0.072
        chem(i,k,j,p_pcg3_b_o) = chem(i,k,j,p_pcg3_b_o) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*0.108
        chem(i,k,j,p_pcg4_b_o) = chem(i,k,j,p_pcg4_b_o) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*0.168
        chem(i,k,j,p_pcg5_b_o) = chem(i,k,j,p_pcg5_b_o) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*0.216
        chem(i,k,j,p_pcg6_b_o) = chem(i,k,j,p_pcg6_b_o) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*0.36
        chem(i,k,j,p_pcg7_b_o) = chem(i,k,j,p_pcg7_b_o) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*0.48
        chem(i,k,j,p_pcg8_b_o) = chem(i,k,j,p_pcg8_b_o) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*0.6
        chem(i,k,j,p_pcg9_b_o) = chem(i,k,j,p_pcg9_b_o) &
        +(emis_ant(i,k,j,p_e_orgi_bb)/1.57+emis_ant(i,k,j,p_e_orgj_bb)/1.57)*conv3*0.96
        chem(i,k,j,p_pcg1_f_o) = chem(i,k,j,p_pcg1_f_o) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*0.0184
        chem(i,k,j,p_pcg2_f_o) = chem(i,k,j,p_pcg2_f_o) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*0.0136
        chem(i,k,j,p_pcg3_f_o) = chem(i,k,j,p_pcg3_f_o) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*0.0208
        chem(i,k,j,p_pcg4_f_o) = chem(i,k,j,p_pcg4_f_o) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*0.032
        chem(i,k,j,p_pcg5_f_o) = chem(i,k,j,p_pcg5_f_o) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*0.0408
        chem(i,k,j,p_pcg6_f_o) = chem(i,k,j,p_pcg6_f_o) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*0.0688
        chem(i,k,j,p_pcg7_f_o) = chem(i,k,j,p_pcg7_f_o) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*0.0936
        chem(i,k,j,p_pcg8_f_o) = chem(i,k,j,p_pcg8_f_o) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*0.12
        chem(i,k,j,p_pcg9_f_o) = chem(i,k,j,p_pcg9_f_o) &
        +(emis_ant(i,k,j,p_e_orgi_a)*2/1.25+emis_ant(i,k,j,p_e_orgj_a)*2/1.25)*conv3*0.192
             end do
          end do
       end do
      else
    ...
    CASE (CBMZ_MOSAIC_4BIN_VBS2_KPP)
     if(config_flags%emiss_opt == 3 ) then
       IF(config_flags%kemit .GT. kte-ksub) THEN
          message = ' EMISSIONS_DRIVER: KEMIT > KME '
...
message)
    ...
    CASE (RACM_SOA_VBS_KPP)
       ...
       call soa_vbs_addemiss( id, dtstep, u10, v10, alt, dz8w, xland, chem, &
            ...
            ims,ime, jms,jme, kms,kme, &
            its,ite, jts,jte, kts,kte )
    CASE (CBMZ_MOSAIC_4BIN, CBMZ_MOSAIC_KPP, CBMZ_MOSAIC_8BIN,CBMZ_MOSAIC_8BIN_VBS9_KPP, &
          CBMZ_MOSAIC_4BIN_AQ, CBMZ_MOSAIC_8BIN_AQ, CBMZ_MOSAIC_4BIN_VBS9_KPP,&
          CBMZ_MOSAIC_DMS_4BIN, CBMZ_MOSAIC_DMS_8BIN, SAPRC99_MOSAIC_4BIN_VBS9_KPP,&
          CBMZ_MOSAIC_DMS_4BIN_AQ, CBMZ_MOSAIC_DMS_8BIN_AQ,SAPRC99_MOSAIC_4BIN_VBS2_KPP,&
          CBMZ_MOSAIC_4BIN_VBS2_KPP,MOZART_MOSAIC_4BIN_VBS0_KPP, &
