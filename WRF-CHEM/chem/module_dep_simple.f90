...
SUBROUTINE wesely_driver( id, ktau, dtstep, config_flags, current_month, &
  ...
  USE module_state_description, only: param_first_scalar
   ...
   REAL, optional, INTENT(IN) :: xice(ims:ime,jms:jme)
   TYPE(grid_config_rec_type), INTENT(IN) :: config_flags
      ...
      if( config_flags%chem_opt /= MOZART_KPP .and. &
          ...
          config_flags%chem_opt /= SAPRC99_MOSAIC_4BIN_VBS2_KPP .and. &
          config_flags%chem_opt /= SAPRC99_KPP .and. &
          config_flags%chem_opt /= SAPRC99_MOSAIC_4BIN_VBS9_KPP .and. &
          config_flags%chem_opt /= SAPRC99_MOSAIC_8BIN_VBS2_AQ_KPP .and. &
          config_flags%chem_opt /= SAPRC99_MOSAIC_8BIN_VBS2_KPP .and. &
...
tile_lon_loop : &
         do i = its,ite
            ...
            if( config_flags%chem_opt == MOZART_KPP .or. &
                ...
                config_flags%chem_opt == SAPRC99_MOSAIC_4BIN_VBS9_KPP .or. &
      ...
      if ( (config_flags%chem_opt == CBMZ ) .or. &
           ...
           (config_flags%chem_opt == CBMZ_SORGAM_KPP ) .or. &
           (config_flags%chem_opt == CBMZ_MOSAIC_KPP ) .or. &
           (config_flags%chem_opt == CBMZ_MOSAIC_4BIN_VBS2_KPP).or.&
           (config_flags%chem_opt == CBMZ_MOSAIC_8BIN_VBS9_KPP ) .or. &
           (config_flags%chem_opt == CBMZ_MOSAIC_4BIN_VBS9_KPP ) .or. &
     ...
     if( config_flags%chem_opt == SAPRC99_MOSAIC_8BIN_VBS2_AQ_KPP ) then
       do j=jts,jte
          do i=its,ite
               ddvel(i,j,p_dms) = 0.
               ddvel(i,j,p_mvk) = 0.
     ...
     if (config_flags%chem_opt == RACM_SOA_VBS_KPP) then
      ...
      if (config_flags%chem_opt == CBMZ_SORGAM_KPP ) then
         do j=jts,jte
            do i=its,ite
               ddvel(i,j,p_aro1) = 0
               ddvel(i,j,p_aro2) = 0
      ...
      if (config_flags%chem_opt == CBMZ_MOSAIC_8BIN_VBS9_KPP ) then
         do j=jts,jte
            do i=its,ite
               ddvel(i,j,p_aro1) = 0
               ddvel(i,j,p_aro2) = 0
      ...
      if (config_flags%chem_opt == CBMZ_MOSAIC_4BIN_VBS9_KPP ) then
         do j=jts,jte
            do i=its,ite
               ddvel(i,j,p_aro1) = 0
               ddvel(i,j,p_aro2) = 0
...
"DEP_SEASONS NOT 5")
        ...
        if( astat /= 0 ) then
           CALL wrf_message( 'ftuv_init: failed to allocate luse2usgs array' )
           CALL wrf_abort
        end if
...
is_cbm4_kpp : &
        ...
        if(p_par > 1) hstar(p_par) = 1.13E-3
        if(p_ch4 > 1) then
           hstar(p_ch4) = 1.50E-3
           dhr(p_ch4) = 0.
           f0(p_ch4) = 0.
