...
'convect_shallow_init: shallow_scheme and eddy_scheme are incompatible' )
    ...
    if( errstring .ne. '' ) call wrf_error_fatal3("<stdin>",1004,&
errstring )
    if( vdiff_select( fieldlist_wet, 'u' ) .ne. '' ) call wrf_error_fatal3("<stdin>",1012,&
vdiff_select( fieldlist_wet, 'u' ) )
    if( vdiff_select( fieldlist_wet, 'v' ) .ne. '' ) call wrf_error_fatal3("<stdin>",1014,&
vdiff_select( fieldlist_wet, 'v' ) )
    if( vdiff_select( fieldlist_wet, 's' ) .ne. '' ) call wrf_error_fatal3("<stdin>",1016,&
vdiff_select( fieldlist_wet, 's' ) )
    ...
    do k = 1, pcnst
       ...
       if( cnst_get_type_byind(k) .eq. 'wet' ) then
          if( vdiff_select( fieldlist_wet, 'q', k ) .ne. '' ) call wrf_error_fatal3("<stdin>",1056,&
