...
subroutine camrad(RTHRATENLW,RTHRATENSW, &
                     ...
                     doabsems, &
                     cu_physics,shallowcu_forced_ra,shall,cubot,cutop, &
                     cldfra_cup, &
   ...
   do j =jts,jte
     ...
     ENDIF
      ...
      case (KFCUPSCHEME, KFETASCHEME)
         if( shall(i,j) == 1 &
              .AND. k >= int(cubot(i,j)) &
              .AND. k <= int(cutop(i,j)) ) q(ii,kk,ixcldliq) = max( 1.0e-3*cldfra_cup(i,k,j), q(ii,kk,ixcldliq) )
      end select
      enddo
      enddo
      do i = its,ite
      ...
      call param_cldoptics_calc(ncol, pcols, pver, pverp, pverr, pverrp, ppcnst, q, cld, landfrac, landm,icefrac, &
                                pdel, t, ps, pmid, pint, cicewp, cliqwp, emis, rel, rei, pmxrgn, nmxrgn, snowh, shall)
...
subroutine radctl(j, lchnk ,ncol , pcols, pver, pverp, pverr, pverrp, ppcnst, pcnst, &
  ...
  subroutine param_cldoptics_calc(ncol, pcols, pver, pverp, pverr, pverrp, ppcnst, &
    ...
    real, intent(in) :: shall(pcols,pver)
