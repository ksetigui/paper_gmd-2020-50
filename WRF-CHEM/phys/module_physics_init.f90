...
CONTAINS
   ...
   REAL, DIMENSION( ims:ime , jms:jme ) , INTENT(OUT) :: &
             RAINC, RAINNC, RAINSH, RAINCV, RAINNCV, RAINSHV, &
             SNOWNC, SNOWNCV, GRAUPELNC, GRAUPELNCV
...
integer myproc
   ...
   if (config_flags%ra_lw_physics .eq. RRTMG_LWSCHEME .and. &
       ...
       config_flags%mp_physics .eq. THOMPSON) then
      has_reqc = 1
      has_reqi = 1
      has_reqs = 1
   endif
 ...
 ENDIF
  ...
  ENDIF
   ...
   CALL shcu_init(STEPCU,CUDT,DT,RUSHTEN,RVSHTEN,RTHSHTEN, &
                RQVSHTEN,RQRSHTEN,RQCSHTEN, &
                RQSSHTEN,RQISHTEN,RQGSHTEN, &
                NCA,RAINSH,RAINSHV,config_flags,restart, &
                allowed_to_read, start_of_simulation, &
                ids, ide, jds, jde, kds, kde, &
                ims, ime, jms, jme, kms, kme, &
                its, ite, jts, jte, kts, kte )
        ...
        DO ls=1,luseas
          ...
          DO LC=1,LUCATS
            IF(found_lu)THEN
              ...
              IF(LC.NE.LI)CALL wrf_error_fatal3("<stdin>",1274,&
...
TRIM ( wrf_err_message ) )
   ...
   ENDIF
   IF ( ( config_flags%o3input .EQ. 2 .OR. config_flags%o3input .EQ. 3 ) .AND. id .EQ. 1 ) THEN
...
'arguments not present for calling urban model' )
      ...
      CASE (NOAHMPSCHEME)
          CALL NOAHMP_INIT(MMINLU, SNOW,SNOWH,CANWAT,ISLTYP,IVGTYP, &
                     TSLB,SMOIS,SH2O,DZS, FNDSOILW, FNDSNOWH, config_flags%isice, &
...
'arguments not present for calling camzmscheme' )
   ...
   REAL , DIMENSION( ims:ime , jms:jme ), INTENT(OUT):: RAINSH, RAINSHV
   REAL , DIMENSION( ims:ime , jms:jme ), INTENT(INOUT):: NCA
  INTEGER :: i,j,itf,jtf
   itf=min0(ite,ide-1)
   jtf=min0(jte,jde-1)
