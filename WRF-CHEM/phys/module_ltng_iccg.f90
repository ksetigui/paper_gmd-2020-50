 ...
 SUBROUTINE iccg_pr93( &
                            kLNB, cldtop_adjustment, t_phy, z, &
                            htop, &
                            shall, &
                            ids, ide, jds, jde, kds, kde, &
                            ims, ime, jms, jme, kms, kme, &
 ...
 jloop: DO j=jps,jpe
    iloop: DO i=ips,ipe
    IF ( total_flashrate(i,j) .gt. 0.) THEN
        if (shall(i,j) .gt. 0.5) then
          depth = -1.
        else
           kfreeze = nint(htop(i,j))
          DO WHILE ( t_phy(i,kfreeze,j) .le. 273.15 )
              kfreeze = kfreeze - 1
          ENDDO
          depth = ( z(i,nint(htop(i,j)),j) - z(i,kfreeze,j) ) * 1E-3 + cldtop_adjustment
          WRITE(message, * ) ' ltng_iccg: cloud depth (km), cldtop, freezelevel = ',depth, htop(i,j), kfreeze, t_phy(i,kps,j),t_phy(i,kpe,j),i,j,kps,kpe
