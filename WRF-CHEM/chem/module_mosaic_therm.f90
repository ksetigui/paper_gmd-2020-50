...
10 return
   ...
   call overall_massbal_in
      ...
      if (istat_mosaic_fe1 .lt. 0) return
      if (istat_mosaic_fe1 .lt. 0) return
      call equilibrium(dtchem,vbs_nbin)
      return
...
2000 continue
      ...
      enddo
        do ibin=1,nbin_a
         enddo
        do i=1,N
           Ctot(i)=total_species(ipcg1_b_c_g+i-1)
           Caer(i)=Paer(ipcg1_b_c_g+i-1)
           Csat(i)=sat_soa(ipcg1_b_c_g+i-1)
           Cgas(i)=gas(ipcg1_b_c_g+i-1)
         enddo
          do i=1,N
            idxfresh(i)=0
            idxaged(i)=0
      ...
      if (nsolaged.gt.0) call soap(nsolaged,Ctotaged, &
       ...
       enddo
        ...
        enddo
          do iv = ipcg1_b_c_g, ngas_volatile
           if (Ctot(iv-ipcg1_b_c_g+1).lt.1d-10) goto 120
            if (total_species(iv).lt.1d-10) goto 120
            dq=gas(iv)-Cgas(iv-ipcg1_b_c_g+1)
           frqtotfresh=0.0d0
           frqtotaged=0.0d0
           mnkfresh=0.0d0
           mnkaged=0.0d0
...
120 continue
        ...
        cpxaged= cpxaged+aer(ioc_a,jp,ibin)
         enddo
         if(cpxaged .eq. 0.0 )then
         print*,"No OA mass available for partitioning - " , &
                "make sure that OA is included in IC/BC"
         stop
         endif
        do i=1,N
           Ctot(i)=total_species(ismpa_g+i-1)
           Caer(i)=Paer(ismpa_g+i-1)
           Csat(i)=sat_soa(ismpa_g+i-1)
           Cgas(i)=gas(ismpa_g+i-1)
         enddo
          do i=1,N
            idxfresh(i)=0
            idxaged(i)=0
      ...
      if (nsolaged.gt.0) call soap(nsolaged,Ctotaged, &
       ...
       enddo
        ...
        stop
         ...
         do ibin=1,nbin_a
           ...
           xsumaged(ibin)=0.0
              xsumaged(ibin)= xsumaged(ibin)+aer(ioc_a,jp,ibin)
         do iv = ismpa_g, ngas_volatile
           if (flagsoap(iv-ismpa_g+1).eq.2) then
               xsumfresh(ibin)= xsumfresh(ibin)+aer(iv,jtotal,ibin)
           elseif (flagsoap(iv-ismpa_g+1).eq.1) then
              xsumaged(ibin)= xsumaged(ibin)+aer(iv,jtotal,ibin)
                elseif (flagsoap(iv-ismpa_g+1).eq.0) then
                 print *, 'Error in mapping flagsoap to ismpa_g'
           endif
         enddo
          if (xsumfresh(ibin).eq.0.0) xsumfresh(ibin)=tinys
          if (xsumaged(ibin).eq.0.0) xsumaged(ibin)=tinys
        enddo
          do iv = ismpa_g, ngas_volatile
           if (Ctot(iv-ismpa_g+1).lt.1d-10) goto 120
            dq=gas(iv)-Cgas(iv-ismpa_g+1)
           frqtotfresh=0.0d0
           frqtotaged=0.0d0
           mnkfresh=0.0d0
           mnkaged=0.0d0
           ...
           if (flagsoap(iv-ismpa_g+1).eq.1) then
              frqaged(ibin)= kg(iv,ibin)*(gas(iv) &
             -(aer(iv,jtotal,ibin))/xsumaged(ibin) &
              *Csat(iv-ismpa_g+1))
          endif
...
11 continue
      ...
      vol_dry_a(ibin) = vol_dry_a(ibin)*1.e-15
        ...
        area_dry_a(ibin)= 0.785398*num_a(ibin)*Dp_dry_a(ibin)**2
        area_wet_a(ibin)= 0.785398*num_a(ibin)*Dp_wet_a(ibin)**2
        ...
        comp_a(join) = aer(ioin_a,jtotal,ibin)*mw_comp_a(join)*1.e-15
         comp_a(jpcg1_b_c)= aer(ipcg1_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg2_b_c)= aer(ipcg2_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg3_b_c)= aer(ipcg3_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg4_b_c)= aer(ipcg4_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg5_b_c)= aer(ipcg5_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg6_b_c)= aer(ipcg6_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg7_b_c)= aer(ipcg7_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg8_b_c)= aer(ipcg8_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg9_b_c)= aer(ipcg9_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg1_b_c)= aer(iopcg1_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg2_b_c)= aer(iopcg2_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg3_b_c)= aer(iopcg3_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg4_b_c)= aer(iopcg4_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg5_b_c)= aer(iopcg5_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg6_b_c)= aer(iopcg6_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg7_b_c)= aer(iopcg7_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg8_b_c)= aer(iopcg8_b_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg1_b_o)= aer(ipcg1_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg2_b_o)= aer(ipcg2_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg3_b_o)= aer(ipcg3_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg4_b_o)= aer(ipcg4_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg5_b_o)= aer(ipcg5_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg6_b_o)= aer(ipcg6_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg7_b_o)= aer(ipcg7_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg8_b_o)= aer(ipcg8_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg9_b_o)= aer(ipcg9_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg1_b_o)= aer(iopcg1_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg2_b_o)= aer(iopcg2_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg3_b_o)= aer(iopcg3_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg4_b_o)= aer(iopcg4_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg5_b_o)= aer(iopcg5_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg6_b_o)= aer(iopcg6_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg7_b_o)= aer(iopcg7_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg8_b_o)= aer(iopcg8_b_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg1_f_c)= aer(ipcg1_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg2_f_c)= aer(ipcg2_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg3_f_c)= aer(ipcg3_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg4_f_c)= aer(ipcg4_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg5_f_c)= aer(ipcg5_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg6_f_c)= aer(ipcg6_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg7_f_c)= aer(ipcg7_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg8_f_c)= aer(ipcg8_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg9_f_c)= aer(ipcg9_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg1_f_c)= aer(iopcg1_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg2_f_c)= aer(iopcg2_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg3_f_c)= aer(iopcg3_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg4_f_c)= aer(iopcg4_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg5_f_c)= aer(iopcg5_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg6_f_c)= aer(iopcg6_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg7_f_c)= aer(iopcg7_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg8_f_c)= aer(iopcg8_f_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg1_f_o)= aer(ipcg1_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg2_f_o)= aer(ipcg2_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg3_f_o)= aer(ipcg3_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg4_f_o)= aer(ipcg4_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg5_f_o)= aer(ipcg5_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg6_f_o)= aer(ipcg6_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg7_f_o)= aer(ipcg7_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg8_f_o)= aer(ipcg8_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jpcg9_f_o)= aer(ipcg9_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg1_f_o)= aer(iopcg1_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg2_f_o)= aer(iopcg2_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg3_f_o)= aer(iopcg3_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg4_f_o)= aer(iopcg4_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg5_f_o)= aer(iopcg5_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg6_f_o)= aer(iopcg6_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg7_f_o)= aer(iopcg7_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jopcg8_f_o)= aer(iopcg8_f_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jant1_c)= aer(iant1_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jant2_c)= aer(iant2_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jant3_c)= aer(iant3_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jant4_c)= aer(iant4_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jant1_o)= aer(iant1_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jant2_o)= aer(iant2_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jant3_o)= aer(iant3_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jant4_o)= aer(iant4_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jbiog1_c)= aer(ibiog1_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jbiog2_c)= aer(ibiog2_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jbiog3_c)= aer(ibiog3_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jbiog4_c)= aer(ibiog4_c_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jbiog1_o)= aer(ibiog1_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jbiog2_o)= aer(ibiog2_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jbiog3_o)= aer(ibiog3_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
         comp_a(jbiog4_o)= aer(ibiog4_o_a,jtotal,ibin)*mw_comp_a(joc)*1.e-15
...
10 if(jp .eq. jsolid)then
 ...
 integer je
      ...
      integer iaer, je, ja, j_index, ibin,jc
     ...
     dens_comp_a(jpcg1_b_c) =1.0
      dens_comp_a(jpcg2_b_c) =1.0
      dens_comp_a(jpcg3_b_c)=1.0
      dens_comp_a(jpcg4_b_c)=1.0
      dens_comp_a(jpcg5_b_c)=1.0
      dens_comp_a(jpcg6_b_c)=1.0
      dens_comp_a(jpcg7_b_c)=1.0
      dens_comp_a(jpcg8_b_c)=1.0
      dens_comp_a(jpcg9_b_c)=1.0
      dens_comp_a(jopcg1_b_c)=1.0
      dens_comp_a(jopcg2_b_c)=1.0
      dens_comp_a(jopcg3_b_c)=1.0
      dens_comp_a(jopcg4_b_c)=1.0
      dens_comp_a(jopcg5_b_c)=1.0
      dens_comp_a(jopcg6_b_c)=1.0
      dens_comp_a(jopcg7_b_c)=1.0
      dens_comp_a(jopcg8_b_c)=1.0
      dens_comp_a(jpcg1_b_o)=1.0
      dens_comp_a(jpcg2_b_o)=1.0
      dens_comp_a(jpcg3_b_o)=1.0
      dens_comp_a(jpcg4_b_o)=1.0
      dens_comp_a(jpcg5_b_o)=1.0
      dens_comp_a(jpcg6_b_o)=1.0
      dens_comp_a(jpcg7_b_o)=1.0
      dens_comp_a(jpcg8_b_o)=1.0
      dens_comp_a(jpcg9_b_o)=1.0
      dens_comp_a(jopcg1_b_o)=1.0
      dens_comp_a(jopcg2_b_o)=1.0
      dens_comp_a(jopcg3_b_o)=1.0
      dens_comp_a(jopcg4_b_o)=1.0
      dens_comp_a(jopcg5_b_o)=1.0
      dens_comp_a(jopcg6_b_o)=1.0
      dens_comp_a(jopcg7_b_o)=1.0
      dens_comp_a(jopcg8_b_o)=1.0
      dens_comp_a(jpcg1_f_c) =1.0
      dens_comp_a(jpcg2_f_c) =1.0
      dens_comp_a(jpcg3_f_c)=1.0
      dens_comp_a(jpcg4_f_c)=1.0
      dens_comp_a(jpcg5_f_c)=1.0
      dens_comp_a(jpcg6_f_c)=1.0
      dens_comp_a(jpcg7_f_c)=1.0
      dens_comp_a(jpcg8_f_c)=1.0
      dens_comp_a(jpcg9_f_c)=1.0
      dens_comp_a(jopcg1_f_c)=1.0
      dens_comp_a(jopcg2_f_c)=1.0
      dens_comp_a(jopcg3_f_c)=1.0
      dens_comp_a(jopcg4_f_c)=1.0
      dens_comp_a(jopcg5_f_c)=1.0
      dens_comp_a(jopcg6_f_c)=1.0
      dens_comp_a(jopcg7_f_c)=1.0
      dens_comp_a(jopcg8_f_c)=1.0
      dens_comp_a(jpcg1_f_o)=1.0
      dens_comp_a(jpcg2_f_o)=1.0
      dens_comp_a(jpcg3_f_o)=1.0
      dens_comp_a(jpcg4_f_o)=1.0
      dens_comp_a(jpcg5_f_o)=1.0
      dens_comp_a(jpcg6_f_o)=1.0
      dens_comp_a(jpcg7_f_o)=1.0
      dens_comp_a(jpcg8_f_o)=1.0
      dens_comp_a(jpcg9_f_o)=1.0
      dens_comp_a(jopcg1_f_o)=1.0
      dens_comp_a(jopcg2_f_o)=1.0
      dens_comp_a(jopcg3_f_o)=1.0
      dens_comp_a(jopcg4_f_o)=1.0
      dens_comp_a(jopcg5_f_o)=1.0
      dens_comp_a(jopcg6_f_o)=1.0
      dens_comp_a(jopcg7_f_o)=1.0
      dens_comp_a(jopcg8_f_o)=1.0
      dens_comp_a(jant1_c)=1.0
      dens_comp_a(jant2_c)=1.0
      dens_comp_a(jant3_c)=1.0
      dens_comp_a(jant4_c)=1.0
      dens_comp_a(jant1_o)=1.0
      dens_comp_a(jant2_o)=1.0
      dens_comp_a(jant3_o)=1.0
      dens_comp_a(jant4_o)=1.0
      dens_comp_a(jbiog1_c)=1.0
      dens_comp_a(jbiog2_c)=1.0
      dens_comp_a(jbiog3_c)=1.0
      dens_comp_a(jbiog4_c)=1.0
      dens_comp_a(jbiog1_o)=1.0
      dens_comp_a(jbiog2_o)=1.0
      dens_comp_a(jbiog3_o)=1.0
      dens_comp_a(jbiog4_o)=1.0
     ...
     integer,intent(in) :: vbs_nbin(1)
      ...
      po_soa(ipcg2_b_c_g) = fn_po(9.91d+4, 83.0d0, T_K)
      po_soa(iopcg1_b_c_g) = fn_po(9.91d-9, 83.0d0, T_K)
      po_soa(ipcg1_b_o_g) = fn_po(9.91d-9, 83.0d0, T_K)
      po_soa(ipcg2_b_o_g) = fn_po(9.91d+4, 83.0d0, T_K)
      po_soa(iopcg1_b_o_g) = fn_po(9.91d-9, 83.0d0, T_K)
      po_soa(ipcg1_f_c_g) = fn_po(9.91d-9, 83.0d0, T_K)
      po_soa(ipcg2_f_c_g) = fn_po(9.91d+4, 83.0d0, T_K)
      po_soa(iopcg1_f_c_g) = fn_po(9.91d-9, 83.0d0, T_K)
      po_soa(ipcg1_f_o_g) = fn_po(9.91d-9, 83.0d0, T_K)
      po_soa(ipcg2_f_o_g) = fn_po(9.91d+4, 83.0d0, T_K)
      po_soa(iopcg1_f_o_g) = fn_po(9.91d-9, 83.0d0, T_K)
