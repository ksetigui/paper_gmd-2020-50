...
SUBROUTINE sedi_1D(QX1d,NX1d,cat,DE1d,iDE1d,iDP1d,gamfact1d,epsQ,epsN,dmx,VxMax,DxMax, &
 ...
 SUBROUTINE mp_milbrandt2mom_main(WZ,T,Q,QC,QR,QI,QN,QG,QH,NC,NR,NY,NN,NG,NH, &
  ...
  integer, parameter :: primIceNucl = 3
  integer, parameter :: optionCNT = 1
  integer, parameter :: immer = 0
  integer, parameter :: contact = 1
  integer, parameter :: homof = 1
  ...
  real, parameter:: CTurn=1.7e11
  real, parameter:: sig_iv0=106.5
  real, parameter:: Rvap=4.615e6
  real, parameter:: RHOIMEAN=0.9
  real, parameter:: akbol=1.3804e-16
  real, parameter:: epsil=0.
  real, parameter:: aeronumi=1.e8
  ...
  REAL :: PODYNTEG4, TEXPTFR2, PODNTEG_DEP, PODNTEG_DEPI
  REAL :: XN2depDT
        ...
        tmp1))))
          ...
          elseif (primIceNucl==3) then
              if (Si>1.) then
                 ...
                 raeros=(dmeandust/2.)*1.e-6
                 densite=pres(i,k)/(287.*T(i,k))
                 dust=dust*densite*1.e-9
                 ...
                 xxx=raeros*100/rgerms
                 fisize=(1.-2.*wetcoef*xxx+xxx**2.)**0.5
                 ...
                 FGERMS=4./3.*3.1416*sig_iv0*(rgerms**2.0)*fshape
                 coefnucl=1.e26*raeros**2.0
                 AJLSHetSD=coefnucl*exp(-(FGERMS)/(akbol*T(i,k)))
                 if (AJLSHetSD.lt.1.e-37) AJLSHetSD=1.e-37
                 AJLSHetS=REAL(AJLSHetSD)
                 TEXPTFR2=1.-exp(-AJLSHetS*dt)
                 PODNTEG_DEP=TEXPTFR2*aeronum
                 PODNTEG_DEP=min(aeronum,PODNTEG_DEP)
                 NuDEPSOR=PODNTEG_DEP
 ...
 1111 continue
              ...
              if (QC(i,k)>epsQ .and. Si>1. .and. Tc<-10.) then
                 ...
                 teta=(-0.0112*tc*tc)+(-0.9566*tc)+7.7245
                 wetcoef=cos((teta*pi)/180.)
                 xxx=raeros*100/rgerms
                 fisize=(1.-2.*wetcoef*xxx+xxx**2.)**0.5
                 xmfi=(xxx-wetcoef)/fisize
                 ...
                 AJLSHetS=REAL(AJLSHetSD)
                 TEXPTFR2=1.-exp(-AJLSHetS*dt)
                 PODNTEG_DEP=TEXPTFR2*aeronumi
                 PODNTEG_DEPI=min(NC(i,k),PODNTEG_DEP)
                 NuDEPSOR=NUDEPSOR+PODNTEG_DEPI
                 if (NuDEPSOR.lt.1e-40) NuDEPSOR=1e-40
 1112 continue
          endif
          endif
